...
var perfectMenu = function (materials, cookbooks, attribute, limit) {
  ...
  const backTracking = (index, arr, attr, stomach) => {
    console.log(index, arr, attr, stomach);

    if (stomach >= limit) {
      if (!arr.some((e) => e < 0)) {
        output = Math.max(output, attr);
      }
      return;
    }
    loop1: for (let i = index; i < cookbooks.length; i++) {
      ...
      attr += attribute[i][0];
      stomach += attribute[i][1];
      backTracking(i, [...arr], attr, stomach);
    }
  };
